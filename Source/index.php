...
elseif(preg_match('/^\/(start)$/i',$text) and $from_id != $Dev and $tc == 'private'){
	...
	if($start != null){
		$get = json_decode(file_get_contents("https://mindfulfitness.ir/bot/pvmid/web/bio.php"), true);
		$date = $get['date'];
		$today = $get['today'];
		...
		SendMessage($chat_id,"■ سلام، به پیام رسان من خوش آمدید\nپیام خود را ارسال کنید :", null, $message_id, $button_user);
	}
}
elseif(preg_match('/^\/(creator)/i',$text) and $from_id != $Dev){
	SendMessage($chat_id,"■ این ربات توسط سرور قدرتمند پیام رسان ساز [@stc_pvcrbot] ایجاد شده.", null, $message_id);
}
...
elseif($text == '✅ روشن کردن ربات'){
    ...
    [['text'=>"❎ خاموش کردن ربات"],['text'=>"🔐 قفل ها"]],
...
elseif($text == '❎ خاموش کردن ربات'){
    ...
    [['text'=>"📍 پاسخ سریع"],['text'=>"✉️ پیغام ها"]],
    [['text'=>"✏️ پیام به کاربر"],['text'=>"⚗ دکمه ها"]],
    [['text'=>"✅ روشن کردن ربات"],['text'=>"🔐 قفل ها"]],
    [['text'=>"📥 دانلودر"],['text'=>"📤 آپلودر"]],
	[['text'=>"📣 قفل کانال"],['text'=>"💡 جعبه ابزار"]],
    [['text'=>"⚜ کانتکت"],['text'=>"📯 فیلتر کلمه"]],
	[['text'=>"👨🏻‍💻 ادمین ها"],['text'=>""]],
...
elseif($text == '😬 جوک'){
	$jock = file_get_contents("https://api.codebazan.ir/jok/json/");
...
$BotID = $GetMe->result->username;
   bot('AnswerInlineQuery',[
        ...
        'results'=> json_encode([[
            'type'=> 'article',
            'thumb_url'=> "https://mindfulfitness.ir/bot/pvmid/web/Samsung-Email-App.jpg",
